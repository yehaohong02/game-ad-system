...
| | 工作坊 | 创意工坊（开发中） |
| | 报表中心 | 日报/周报自动生成 |
| 管理者模式 | 管理者看板 | 团队整体 KPI 看板、花费/展示/点击/CPC/CTR 汇总 |
| | 数据诊断（团队体检报告） | Excel 导入设计师素材表，自动生成团队体检报告 |
| | 创意洞察 | 设计师素材表现对比、效率分析 |
| | 执行闭环 | 团队执行状态管理 |
| | 安全防护 | 团队预算安全监控 |
| | 记忆沉淀 | 团队经验知识库 |
...
**团队体检报告功能：** 导入包含"设计师"和"媒体"字段的 Excel 素材表后，自动生成：
- 团队整体概览（总花费、总展示、总点击、CPC、CTR）
- 设计师综合排名（体量 40% + 效率 35% + 质量 25%）
- 每位设计师深度分析卡片（优势/劣势/短期动作/长期方向）
- 设计师分级（S/A/B/C 四级，基于 CPC 阈值）
- 渠道预算重分配建议
- 视频素材质量评分体系
- 预期收益预测

**主题系统：** 支持 Candy（糖果色）和 Tech（科技蓝）双主题切换，基于 CSS 变量 + antd ConfigProvider 实现全局主题动态切换。

### 数据导入指南

**管理者模式数据诊断** 支持通过 Excel 导入素材数据进行团队体检分析。

**导入步骤：**
1. 在左侧菜单切换到管理者模式（点击 TopBar 模式切换）
2. 进入"买量表格数据"页面
3. 点击右上角 **"导入Excel"** 按钮
4. 选择符合格式的 Excel 文件（需包含 `素材ID`、`设计师`、`媒体` 等列）
...
|------|------|------|------|
| 素材ID | string | 是 | 唯一标识 |
| 设计师 | string | 是 | 所属设计师名称 |
| 媒体 | string | 是 | 投放渠道（Google/FaceBook/TikTok 等） |
| 素材花费 | number | 否 | 广告花费（USD） |
| 素材展示数 | number | 否 | 广告展示次数 |
| 素材点击数 | number | 否 | 广告点击次数 |
| 素材点击率 | number | 否 | CTR（小数形式，如 0.015 = 1.5%） |
| 素材千次展示成本 | number | 否 | CPM |
| 播放次数 | number | 否 | 视频播放次数 |
| 播放2s/6s/25%/50%/75%/100%次数 | number | 否 | 各阶段播放次数 |

> 提示：不清楚格式时，可直接联系管理员获取模板文件。

**数据安全：**
- 导入的数据存储在浏览器 localStorage 中
- 点击"清除数据"可恢复默认演示数据
- 敏感数据不会上传到服务器
